...
记矩阵$\Av$的$n$个列分别为$\av_1, \ldots, \av_n$，由于$\rank(\Av) = m$，因此可以从中挑出$m$个\blue{线性无关}的列$\av_{i_1}, \ldots, \av_{i_m}$构成基$\Bv$，这些列也称为\blue{基向量}。为表述方便，引入矩阵的切片表示$\Av_{\Rcal,\Ccal}$，其中$\Rcal$、$\Ccal$为索引集合，例如记$\Bcal = \{i_1, \ldots, i_m\}$、$\Dcal = [n] \setminus \Bcal$，则
\begin{align*}
    \Bv = \Av_{:,\Bcal} =
    \begin{bmatrix}
    ...
    \end{bmatrix}
\end{align*}
同理$\Dv = \Av_{:,\Dcal} \in \Rbb^{m \times (n-m)}$是\blue{非基向量}构成的矩阵，进一步有
\begin{align*}
    \bv = \Av \xv = \Bv \xv_\Bcal + \Dv \xv_\Dcal
\end{align*}
其中$\xv_\Bcal$称为\blue{基变量}，$\xv_\Dcal$称为\blue{非基变量}。令$\xv_\Dcal = \zerov$可得$\xv_\Bcal = \Bv^{-1} \bv$，这就得到了$\Av \xv = \bv$在基$\Bv$下的\blue{基本解}。
\begin{itemize}
    \item 如果基本解中某些基变量为零，则称其为\blue{退化的基本解}；
    \item 如果基本解还是线性规划的可行解(所有变量非负)，则称其为\blue{基本可行解}。
\end{itemize}
...
\begin{proof}
    1. 设$\xv$是一个可行解，其中$\{ i \mid x_i > 0, ~ i \in [n] \} = \{ i_1, \ldots, i_l \} \triangleq \Lcal$，即$\xv$有$l$个正分量，则
