...
$v^{(k)}$：从源点$s$出发经过不超过$k$条边到达点$v$的最短路径的长度
...
$\ell_{ij}^{(m)}$：从点$i$出发经过不超过$m$条边到达点$j$的最短路径的长度

$$
\begin{align*}
    \quad \ell_{ij}^{(m)} = \begin{cases} \infty, & m = 0 \\ \min \{ \ell_{ij}^{(m-1)}, ~ \min_k \{ \ell_{ik}^{(m-1)} + w(u,v) \} \}, & 0 < m \le n-1 \end{cases}
\end{align*}
$$
