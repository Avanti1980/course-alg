...
@import "../dot/sorting/merge-sort.dot" {.left40 .top-10}
...
@import "../codes/intro/sorting.py" {line_begin=26 line_end=65 .left4 .line-numbers .top0}

@import "../dot/sorting/merge-merge.dot" {.left50per .top-50per}
...
$$
\begin{align*}
    \quad & \spadesuit \qquad 2 \cdot T (2^{k-1}) + 2^{k-1} \le T(2^k) \le 2 \cdot T (2^{k-1}) + 2^k \\
    & \heartsuit \qquad 2 \cdot T (2^{k-2}) + 2^{k-2} \le T(2^{k-1}) \le 2 \cdot T (2^{k-2}) + 2^{k-1} \\
    & \qquad \qquad \vdots \\
    & \clubsuit \qquad 2 \cdot T (2^0) + 2^0 \le T(2^1) \le 2 \cdot T (2^0) + 2^1
\end{align*}
$$
...
@import "../tikz/max-subarray/max-subarray.svg" {.center .width90}
...
<p class="fragment">问题：跨越中点的逆序对可能有$\Theta(n^2)$个，若一个个数，复杂度至少为$\Omega(n^2)$，如何加速？</p>
...
@import "../codes/dc/count-inverse-pair.py" {line_begin=12 line_end=56 .left4 .line-numbers .top0 .bottom1 highlight=[11,20,33,39-42,44]}
...
$$
\begin{align*}
    \quad T(n) & = \begin{cases} 1, & n = 1 \\ 2 \cdot T (n/2) + \Theta(n), & n > 1 \end{cases} \\
    & = \Theta(n \lg n)
\end{align*}
$$
...
@import "../tikz/closest-pair/closest-pair.svg" {.center .width60 .top5}
...
@import "../codes/dc/closest-pair.py" {line_begin=6 line_end=82 .left4 .line-numbers .top0 highlight=[]}
...
@import "../codes/dc/closest-pair.svg" {.center .width75 .top4 .bottom-4}
...
$$
\begin{align*}
    \quad \sum_{k=0}^{2n-1} Z[k] 10^k & = \left( \sum_{i=0}^{n-1} X[i] 10^i \right) \left( \sum_{j=0}^{n-1} X[j] 10^j \right) = \sum_{i=0}^{n-1} \sum_{j=0}^{n-1} X[i] Y[j] 10^{i+j} \\
    & = \sum_{k=0}^{2n-2} \underbrace{\sum_{(i,j): i+j=k} X[i] Y[j]}_{c_k} 10^k = \sum_{k=0}^{2n-2} c_k 10^k
\end{align*}
$$
...
<p class="fragment top4">$Z[k]$和$c_k$之间的关系？</p>
...
\begin{align*}
    \quad \begin{cases}
        c_0 ~ \mathrm{mod} ~ 10, & \text{就是左边$10^0$的系数$Z[0]$} \\
        h \leftarrow \lfloor c_0 / 10 \rfloor, & \text{作为进位参与$Z[1]$的计算}
    \end{cases}
\end{align*}
$$
...
\begin{align*}
    \quad \begin{cases}
        (c_1 + h) ~ \mathrm{mod} ~ 10, & \text{就是左边$10^1$的系数$Z[1]$} \\
        h \leftarrow \lfloor (c_1 + h) / 10 \rfloor, & \text{作为进位参与$Z[2]$的计算}
    \end{cases}
\end{align*}
$$
...
4. &emsp;&emsp;&emsp;&emsp;$c \leftarrow h + X[i] Y[j]$
5. &emsp;&emsp;{==end for==}
6. &emsp;&emsp;$Z[k] = c ~ (\modd 10)$
7. &emsp;&emsp;下一位的进位$c \leftarrow \lfloor c/10 \rfloor$
8. {==end for==}

<p class="fragment top4" data-fragment-index="1">算法第 2、3 行的二重 for 循环共遍历$n^2$个$(i,j)$二元组，即二维表中的每一格，因此算法时间复杂度为$\Theta(n^2)$<span class="fragment">，更好的算法？</span></p>

@import "../tikz/integer-multiplication/integer-multiplication.svg" {.right15 .lefta .width25 .top-45per}
...
$$
\begin{align*}
    \quad T(n) = 4 \cdot T (n/2) + \Theta(n) \Longrightarrow T(n) = \Theta(n^2)
\end{align*}
$$

<p class="fragment">分解成$4$个规模大致减半的子问题并不是更优的算法</p>
...
$$
\begin{align*}
    \quad x y = a c \cdot 10^{2m} + ((a+b) (c+d) - a c - b d) \cdot 10^m + b d
\end{align*}
$$
...
$$
\begin{align*}
    \quad T(n) = 3 \cdot T (n/2) + \Theta(n) \Longrightarrow T(n) = \Theta(n^{\log_2 3})
\end{align*}
$$

<p class="fragment">分解成$3$个规模大致减半的子问题可以得到更优的算法</p>
...
@import "../codes/intro/integer-multiplication-karatsuba.svg" {.center .width75 .top2 .bottom-4}
...
$$
\begin{align*}
    \quad x & = a \cdot 10^{2m} + b \cdot 10^m + c \\
...
$$
\begin{align*}
    \quad xy = a d \cdot 10^{4m} + (a e + b d) \cdot 10^{3m} & + (af + be + cd) \cdot 10^{2m} \\
    & + (bf + ce) \cdot 10^m + cf
\end{align*}
$$
...
$$
\begin{align*}
    \quad x(t) y(t) & = a d \cdot t^4 + (a e + b d) \cdot t^3 + (af + be + cd) \cdot t^2 + (bf + ce) \cdot t + cf \\
    & \triangleq w_4 t^4 + w_3 t^3 + w_2 t^2 + w_1 t + w_0 \triangleq w(t)
\end{align*}
$$
...
@import "../codes/intro/integer-multiplication-all.svg" {.center .width75 .top2 .bottom-4}
...
@import "../codes/dc/matrix-addition.py" {line_begin=3 line_end=7 .left4 .line-numbers .top1 .bottom1 highlight=[2-4]}
...
$$
\begin{align*}
    \quad T(n) = \begin{cases} 1, & n = 1 \\ 4 \cdot T(n/2) + \class{blue}{\Theta(n^2)}, & n > 1 \end{cases}
\end{align*}
$$
...
@import "../codes/dc/matrix-addition.py" {line_begin=9 line_end=26 .left4 .line-numbers .top0}
...
$$
\begin{align*}
    \quad & \spadesuit \qquad T(2^k) = 4 \cdot T (2^{k-1}) + c 4^k \\
    & \heartsuit \qquad T(2^{k-1}) = 4 \cdot T (2^{k-2}) + c 4^{k-1} \\
    & \qquad \qquad \vdots \\
    & \clubsuit \qquad T(2^1) = 4 \cdot T (2^0) + c 4^1
\end{align*}
$$
...
$$
\begin{align*}
    \quad T(n) = \begin{cases} 1, & n = 1 \\ 4 \cdot T(n/2) + c, & n > 1 \end{cases}
\end{align*}
$$
...
$$
\begin{align*}
    \quad & \spadesuit \qquad T(2^k) = 4 \cdot T (2^{k-1}) + c \\
    & \heartsuit \qquad T(2^{k-1}) = 4 \cdot T (2^{k-2}) + c \\
    & \qquad \qquad \vdots \\
    & \clubsuit \qquad T(2^1) = 4 \cdot T (2^0) + c
\end{align*}
$$
...
@import "../codes/dc/matrix-multiply.py" {line_begin=3 line_end=8 .left4 .line-numbers .top1 .bottom1 highlight=[2-5]}
...
$$
\begin{align*}
    \quad T(n) = \begin{cases} 1, & n = 1 \\ 8 \cdot T(n/2) + \class{blue}{\Theta(n^2)}, & n > 1 \end{cases}
\end{align*}
$$
...
@import "../codes/dc/matrix-multiply.py" {line_begin=10 line_end=30 .left4 .line-numbers .top0}
...
$$
\begin{align*}
    \quad & \spadesuit \qquad T(2^k) = 8 \cdot T (2^{k-1}) + c 4^k \\
    & \heartsuit \qquad T(2^{k-1}) = 8 \cdot T (2^{k-2}) + c 4^{k-1} \\
    & \qquad \qquad \vdots \\
    & \clubsuit \qquad T(2^1) = 8 \cdot T (2^0) + c 4^1
\end{align*}
$$
...
$$
\begin{align*}
    \quad T(n) = \begin{cases} 1, & n = 1 \\ 8 \cdot T(n/2) + c, & n > 1 \end{cases}
\end{align*}
$$
...
$$
\begin{align*}
    \quad & \spadesuit \qquad T(2^k) = 8 \cdot T (2^{k-1}) + c \\
    & \heartsuit \qquad T(2^{k-1}) = 8 \cdot T (2^{k-2}) + c \\
    & \qquad \qquad \vdots \\
    & \clubsuit \qquad T(2^1) = 8 \cdot T (2^0) + c
\end{align*}
$$
...
关键：如何将$\widetilde{\Av}$可以分解成$m$个{==秩$1$矩阵==}的和，其中$m < 8$
...
@import "../codes/dc/matrix-multiply.py" {line_begin=32 line_end=88 .left4 .line-numbers .top0}
...
@import "../tikz/recursion-tree/recursion-tree1.svg" {.center .width80 .top3}
...
@import "../tikz/recursion-tree/recursion-tree2.svg" {.center .width80 .top0 .bottom4}
...
$$
\begin{align*}
    \quad & \spadesuit \qquad T(4^k) = 3 \cdot T(4^{k-1}) + c \cdot 16^k \\
    & \heartsuit \qquad T(4^{k-1}) = 3 \cdot T (4^{k-2}) + c \cdot 16^{k-1} \\
    & \qquad \qquad \vdots \\
    & \clubsuit \qquad T(4^1) = 3 \cdot T (4^0) + c \cdot 16^1
\end{align*}
$$

<div class="top-2"></div>

令$\spadesuit + \heartsuit \cdot 3 + \cdots + \clubsuit \cdot 3^{k-1}$可得

$$
\begin{align*}
    \quad T(n) & = 3^k \cdot T(1) + c \cdot 16^k \left( 1 + \frac{3}{16} + \cdots + \frac{3^{k-1}}{16^{k-1}} \right) \\
    & = \underbrace{n^{\log_4 3} \cdot T(1)}_{\text{叶结点}} + \underbrace{c n^2 \sum_{i=0}^{k-1} \left( \frac{3}{16} \right)^i}_{\text{内部结点}}
\end{align*}
$$
...
@import "../tikz/recursion-tree/recursion-tree-imbalance.svg" {.center .width60 .top3}
...
3. 若$\exists \epsilon > 0$使得$f(n) = \Omega(n^{\log_b a + \epsilon})$且$f$满足{==正则条件==}：对$c < 1$和充分大的$x$有$a f(x/b) \le c f(x)$，则$T(n) = \Theta(f(n))$
...
- 情形 3 中$f(n)$占上风，若进一步满足{==正则条件==}，它主导最终的界
...
- 若$a > b^d$，则$\log_b a > d$，即情形 1，等比数列公比$q>1$，和不超过最后一项$(a/b^d)^{t-1} = a^{t-1} b^d / n^d < a^t / n^d = a^{\log_b n}/ n^d = n^{\log_b a} / n^d$的常数倍，故$g(n) = \Theta(n^{\log_b a})$
- 若$a = b^d$，则$\log_b a = d$，即情形 2 中$k=0$的情况，等比数列每项都是$1$，和为$t = \log_b n$，故$g(n) = \Theta(n^{\log_b a} \lg n)$
- 若$a < b^d$，则$\log_b a < d$，即情形 3，等比数列公比$q<1$，和不超过$1/(1-q)$，若$q$不能无限接近$1$，可视为常数 (正则条件正是为了保证这一点)，故$g(n) = \Theta(n^d) = \Theta(f(n))$
...
$$
\begin{align*}
    \quad \int_0^t h(x) \diff x = \left. \frac{1}{k+1} x^{k+1} \right|_0^t = \frac{1}{k+1} t^{k+1} = \frac{1}{k+1} \log_b^{k+1} n
\end{align*}
$$
