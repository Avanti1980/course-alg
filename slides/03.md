...
- 若$a = b^d$，则$\log_b a = d$，即情形 2，等比数列每项都是$1$，和为$t$，故$g(n) = \Theta(n^{\log_b a} \log_b n)$
- 若$a < b^d$，则$\log_b a < d$，即情形 3，等比数列公比$q<1$，和不超过$1/(1-q)$，若$q$不能无限接近$1$，可视为常数 (正则条件)，故$g(n) = \Theta(n^d) = \Theta(f(n))$
