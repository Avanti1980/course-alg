...
- 若$a > b^d$，则$\log_b a > d$，即情形 1，等比数列公比$q>1$，和为最后一项$(a/b^d)^{t-1} = a^{t-1} b^d / n^d < a^t / n^d = a^{\log_b n}/ n^d = n^{\log_b a} / n^d$的常数倍，故$g(n) = \Theta(n^{\log_b a})$
- 若$a = b^d$，则$\log_b a = d$，即情形 2，等比数列公比$q=1$，和为$t$，故$g(n) = \Theta(n^{\log_b a} \log_b n)$
- 若$a < b^d$，则$\log_b a < d$，即情形 3，等比数列公比$q<1$，和不超过$1/(1-q)$，若$q$不非常接近$1$，可以视为常数 (正则条件)，故$g(n) = \Theta(n^d) = \Theta(f(n))$
