...
> 内层 for 循环：每次进入循环前，子数组$a[j, \ldots, n-1]$的元素与原来的子数组$a[j, \ldots, n-1]$相同 (顺序可能有差别)，且第一个元素最小

> 外层 for 循环：每次进入循环前，子数组$a[0, \ldots, i-1]$已排好序，且其元素小于子数组$a[i, \ldots, n-1]$的元素

最终循环终止时有$i = n-1$，根据外层的循环不变式，此时子数组$a[0, \ldots, n-2]$已排好序且其元素小于子数组$a[n-1]$的元素
...
1. 初始：子数组只有$a[n-1]$一个元素，满足元素相同且第一个元素最小
2. 保持：每轮比较$a[j-1]$和$a[j]$并将小者置于前，因此每轮迭代结束后子数组$a[j, \ldots, n-1]$长度会加$1$(显然保持元素相同)，且第一个元素最小
3. 终止：循环终止时有$j = i$，此时子数组$a[i, \ldots, n-1]$第一个元素最小
...
1. 初始：子数组只有$a[0, \ldots, -1]$为空，循环不变式显然成立
2. 保持：根据内层 for 循环的终止情况，$a[i, \ldots, n-1]$第一个元素最小，又子数组$a[0, \ldots, i-1]$已排好序且其元素小于$a[i, \ldots, n-1]$的元素，因此子数组$a[0, \ldots, i]$亦排好序且其元素小于$a[i+1, \ldots, n-1]$的元素
3. 终止：循环终止时有$i = n-1$，此时数组$a[0, \ldots, n-1]$已排好序
...
- 第 6 行的比较操作在二重循环里，因此算法总时间$\in \Theta(n^2)$
- 第 9 行的交换操作在一重循环里，因此最坏情况下交换次数$\in \Theta(n)$
