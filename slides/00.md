...
- $\gcd(a,b) = \gcd(b,r)$，不断令$(a,b) = (b,r)$直到$r = 0$
...
- 暴力穷举法迭代次数$\min(a, b)$，每轮做$2$次取模运算
- 辗转相除法迭代次数约为$\log(\max(a,b))$，每轮做$1$次取模运算
...
最坏情况下迭代次数为$\max(a, b)$，例如$\gcd(10000,1)$
...
迭代次数约为$\log(\max(a, b))$，同辗转相除，但避免了取模运算
...
输入数组$A = \langle a_1, a_2, \ldots, a_n \rangle$，输出$a'_1 \le a'_2 \le \cdots \le a'_n$
