...
$$
\begin{align*}
    \quad l(p) = \sum_{i=1}^k w(v_{i-1}, v_k)
\end{align*}
$$
...
$$
\begin{align*}
    \quad \delta(u,v) = \begin{cases} \min \{ l(p) : u \overset{p}{\rightsquigarrow} v \}, & \text{如果存在 } u \text { 到 } v \text{ 的路径} \\ \infty, & \text{其它} \end{cases}
\end{align*}
$$
...
$d_v^{(k)}$：从$s$出发经过不超过$k$条边到达点$v$的最短路径的长度
...
- 没有收益，此时$d_v^{(k)} = d_v^{(k-1)}$
- 有收益，经过$k-1$条边到点$u$后再到$v$，$d_v^{(k)} = d_u^{(k-1)} + w(u,v)$

<div class="top4"></div>

$$
\begin{align*}
    \quad d_v^{(k)} = \begin{cases} \infty, & k = 0 \\ \min \{ d_v^{(k-1)}, ~ \min_u \{ d_u^{(k-1)} + w(u,v) \} \}, & 0 < k \le |\Vcal|-1 \end{cases}
\end{align*}
$$
...
$$
\begin{align*}
    d_v^{(k)} = \begin{cases} \infty, & k = 0 \\ \min \{ d_v^{(k-1)}, ~ \min_u \{ d_u^{(k-1)} + w(u,v) \} \}, & 0 < k \le |\Vcal|-1 \end{cases}
\end{align*}
$$
...
导论书上称$d_v \leftarrow \min \{ d_v, ~ d_u + w(u,v) \}$为{==松弛==} (relax)
