...
路径$p = \langle v_0, v_1, \ldots, v_k \rangle$的长度$l(p)$是路径上所有边的权重之和

$$
\begin{align*}
    \quad l(p) = \sum_{i=1}^k w(v_{i-1}, v_i)
\end{align*}
$$
...
$$
\begin{align*}
    \quad \delta(u,v) = \begin{cases} \min_p \{ l(p) : u \overset{p}{\rightsquigarrow} v \}, & \text{如果存在 } u \text { 到 } v \text{ 的路径} \\ \infty, & \text{其它} \end{cases}
\end{align*}
$$
...
设最短路径是{==简单路径==}，即不包含环路、最多有$|\Vcal|-1$条边
...
若无负环，第$i$轮恰可求得最短路径边数为$i$的点的最短路径
...
区别：{==同一轮先更新的$d_u$本轮可以直接用==}

任意点的$d$值单调递减，最新的就是最好的，用最新的没毛病

如果本轮$d_u$的值更新了，$d_v$值也可以跟着更新，加速求解，只不过该算法不能再叫动态规划了

在动态规划的实现中，$d_v$值必须到下一轮才能更新，因为本轮用的还是上一轮的$d_u$
...
当采用即时更新时，点的更新顺序会产生影响

@import "../dot/chain.dot" {.top0}

@import "../codes/bellman-ford-compare.py" {line_begin=0 line_end=85 .left4 .line-numbers .top0 highlight=[8,11,25]}
