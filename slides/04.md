...
输入元素各不相同的序列$X$，输出$X$的最长递增子序列 (LIS)

最优子结构性：设以$X[i]$结尾的 LIS 为$X[i_1], \ldots, X[i_k], X[i]$，则$X[i_1], \ldots, X[i_k]$必然是以$X[i_k]$结尾的 LIS

对于$X[i]$，在以$X[0], X[1], \ldots, X[i-1]$结尾的所有 LIS 中，选择{==最长==}且{==可接==}的，接在后面即可

记$d[i]$为以$X[i]$结尾的 LIS 的长度，递推关系：
...
设$e[i]$是所有长度为$i$的 LIS 中末位元素的{==最小==}值，初始为空
...
$e$的长度为 4 代表 LIS 的<span class="blue">长度为 4</span>，注意$e$本身并不是 LIS
...
- 要么得到一个更长的 LIS
- 要么让现有的某个 LIS 的末位变小，这就是贪心

<div class="top2"></div>

贪心：对各种长度的 LIS 只维护末位最小的，给后面留更多余地

<!-- slide data-notes="" -->

##### <span style="font-weight:900">LIS</span> 贪心 单调性

---

设$e[i]$是所有长度为$i$的 LIS 中末位元素的{==最小==}值，初始为空

<span class="blue">在任意时刻$e$都是严格单调递增的</span>
...
##### <span style="font-weight:900">LIS</span> 贪心 递推关系
...
- 若$e[-1] < X[i]$，将其接到$e$的后面，此时找到了更长的 LIS
- 若$e[j-1] < X[i] < e[j]$，则令$e[j] = X[i]$，长度为$j$的 LIS 变得更优了
...
二分查找时间复杂度为$O(\log n)$，总时间复杂度为$O(n \log n)$
