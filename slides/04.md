...
既然$F$的计算顺序为$F(2), F(3), \ldots, F(n)$，为何不直接用循环顺序填写？
...
<ul>
    <li class="fragment">在<span class="blue"><code>fibo_rec_faster()</code></span>中，每次递归，需要计算若干次两个相邻斐波那契数的乘积，目前最快的两个$n$位数的乘法时间开销为$O(n \lg n)$，因此$T(n) = T(n/2) + O(n \lg n)$，根据主定理情形 3，$T(n) = \Theta(n \lg n)$</li>
