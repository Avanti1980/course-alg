...
设$T(n)$为调用 fibo_rec() 的次数

$$
\begin{align*}
    \quad T(0) & = T(1) = 1, ~ \ldots, ~ T(n) = T(n-1) + T(n-2) + 1 \\[10pt]
    \Longrightarrow & ~ \underbrace{T(n) + 1}_{G(n)} = \underbrace{T(n-1) + 1}_{G(n-1)} + \underbrace{T(n-2) + 1}_{G(n-2)}, ~ \ldots, ~ G(0) = G(1) = 2 \\[10pt]
    \Longrightarrow & G(n) = 2 F(n+1) \\[10pt]
    \Longrightarrow & T(n) = 2 F(n+1) - 1
\end{align*}
$$

<p class="fragment">$F(n) = \frac{1}{\sqrt{5}} ( (\frac{1 + \sqrt{5}}{2})^n - (\frac{1 - \sqrt{5}}{2})^n )$，指数增长</p>
...
每个子问题第一次求解后将结果存下来，再用时直接查表

@import "../codes/fibo.py" {line_begin=7 line_end=21 .left4 .line-numbers .top-2 .bottom0 highlight=[2,10-11]}

<p class="fragment">备忘录英文为Memoization，注意i前面没有r，由英国人工智能先驱<a href="https://en.wikipedia.org/wiki/Donald_Michie" target="_blank">唐纳德·米奇</a> (Donald Michie) 于1967年提出，但其实米奇本人只提出了memo function的概念，并没有用这个词，而更早的是香农在他1950年制造的机械鼠Theusus中就使用该词了</p>
...
动态规划 (dynamic programming, DP) 由美国数学家<a href="https://en.wikipedia.org/wiki/Richard_E._Bellman" target="_blank">理查德·贝尔曼</a> (Richard E. Bellman) 于 50 年代中期提出
...
<p class="fragment">事实上贝尔曼首次用动态规划这个词是在1952年，早于威尔逊去五角大楼任职数月</p>
...
<p class="fragment">$T(n) = T(\lfloor n/2 \rfloor) + f(n)$，其中$f(n) = O(n \lg n)$是两个$n$位数的乘法时间开销，根据主定理情形3，$T(n) = \Theta(f(n))$</p>

<p class="fragment">前面一重循环中有两个$n$位数相加，实际时间复杂度为$O(n^2)$</p>

<p class="fragment"><i class="fa fa-lightbulb-o"></i> 动态规划未必一定比分治递归快，当分治递归产生的子问题个数更少且没有重复子问题时，动态规划就没有优势了</p>
...
1. 确定保存子问题结果的数据结构：多维表格、……
2. 除边界条件外，子问题相互间的依赖关系(偏序)
3. 根据上一步的依赖关系确定子问题的求解顺序(偏序=>线序)
4. 若除最优解的值外还需最优解本身，在第 1 步里维护一些额外信息
...
设长度为$n$的钢条的最优切割方案为$n = n_1 + n_2 + \cdots + n_k$

对$\forall \Ical = \{ i_1, \ldots, i_l \} \subseteq \{1, \ldots, k\}$、$\{ i_{l+1}, \ldots, i_k \} = \{1, \ldots, k\} \setminus \Ical$

- $n_{i_1} + \cdots + n_{i_l} = m$也是长度为$m$的钢条的最优切割方案
- $n_{i_{l+1}} + \cdots + n_{i_k} = n - m$也是长度为$n-m$的钢条的最优切割方案

<div class="top2"></div>

反设$n'_1 + \cdots + n'_j = m$是长度为$m$的更优切割方案

则$n'_1 + \cdots + n'_j + n_{i_{l+1}} + \cdots + n_{i_k}$是长度为$n$的更优切割方案
...
- 分：将钢条切成长度分别为$m$、$n-m$的两段
- 治：分别求这两段的最优切割方案对应的收益$r_m$、$r_{n-m}$
- 合：$r'_n = r_m + r_{n-m}$

<div class="top2"></div>

$r'_n$就是最优解吗？分割为$m$和$n-m$两段未必是最优的，但遍历所有可能的分割取最大肯定是最优的
...
@import "../dot/cr-rec.dot" {.center .top0}
...
@import "../codes/cut-rod.py" {line_begin=26 line_end=40 .left4 .line-numbers .top1 .bottom0 highlight=[2,7,12-13]}
...
@import "../codes/cut-rod.py" {line_begin=52 line_end=65 .left4 .line-numbers .top0 .bottom0 highlight=[2,8,11-13]}
...
<p class="fragment"><i class="fa fa-lightbulb-o"></i> 不要无脑动态规划，它并不是总能有提升的</p>
...
<p class="fragment"><i class="fa fa-comments"></i> $P(n)$称为<a href="https://en.wikipedia.org/wiki/Catalan_number" target="_blank">卡特兰数</a> (Catalan number)</p>
...
<p class="fragment">最长路径的两个子问题不独立，子问题 ① 到 ② 的最长路径经过的点，在子问题 ② 到 ③ 的最长路径中不能再经过</p>
