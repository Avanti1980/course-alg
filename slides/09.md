...
对最小 (大) 化问题，用估计的下 (上) 界剪枝

- 对确定了部分分量的解，{==估计==}其对目标函数取值的{==下界==}
- 一旦得到一个可行解，所有下界大于其目标函数值的结点被剪枝

<div class="top2"></div>

估计上下界的函数就是限界函数，其设计是分支限界的关键
...
<div class="left8 righta top-2 tighttable row1-4-column2-5-fullborder fs10">
...
<div class="left8 righta top-2 row1-4-column2-5-fullborder fs10">
...
对任一确定了部分分量的解，下界为：已分派任务的成本和 + 剩余每行与已分派任务不冲突的最小值

- 若给人员 1 分派任务 1，成本下界 = 9 + 3 + 1 + 4 = 17
- 若给人员 1 分派任务 2，成本下界 = 2 + 3 + 1 + 4 = 10，潜力最大
- 若给人员 1 分派任务 3，成本下界 = 7 + 4 + 5 + 4 = 20
- 若给人员 1 分派任务 4，成本下界 = 8 + 3 + 1 + 6 = 18
...
@import "../dot/job-distribution.dot" {.top-4 .bottom0 .left1per}

<div class="top0"></div>

左下角的结点对应分派 (2, 1, 3, 4)，成本 13，其它活结点的成本下界都高于 13，可全部杀死，最优解就是 13
...
对任一确定了部分分量的解，上界为：已选物品总价值 + 剩余承重量全部采用剩余物品中价值/重量最大的物品的总价值
...
右下角的结点对应取物品 1 和物品 3，总价值 65，其它活结点的价值上界都低于 65，可全部杀死，最优解就是 65

</div>

@import "../dot/bag.dot" {.right2 .lefta .top-45per .bottom-10}
