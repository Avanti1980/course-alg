...
@import "../codes/matrix-multiply.py" {line_begin=71 line_end=113 .left4 .line-numbers .top1}
...
例：$T(n) = 3 \cdot T(n/4) + \Theta(n \lg n)$

$a = 3$、$b = 4$、$f(n) = \Theta(n \lg n)$，$n^{\log_b a} = n^{\log_4 3} \approx n^{0.793}$

若属于情形 3，则$T(n) = \Theta(n \lg n)$
...
考虑函数$h(n) = (n-1)^k$的黎曼积分，易知$\sum_{i=1}^t i^k$是

$$
\begin{align*}
    \quad \int_1^{t+1} h(n) \diff n = \left. \frac{1}{k+1} (n-1)^{k+1} \right|_1^{t+1} = \frac{1}{k+1} t^{k+1}
\end{align*}
$$
...
情形 3：若$\exists \epsilon > 0$使得$f(n) = \Omega(n^{\log_b a + \epsilon})$且$f(n)$满足{==正则条件==}：对$c < 1$和充分大的$n$有$a f(n/b) \le c f(n)$，则

$$
\begin{align*}
    \quad g(n) & = \sum_{i=0}^{t-1} a^i f \left( \frac{n}{b^i} \right) \le \sum_{i=0}^{t-1} c^i f(n) \le f(n) \sum_{i=0}^\infty c^i = \frac{f(n)}{1-c} = O(f(n))
\end{align*}
$$
...
我的批注 这里省略了对{==非充分大==}的$n$的处理，它们的影响在有限的常数范围内，对$O(f(n))$的界没有影响
