...
两个没有刻度的水桶，总容量分别为$a$升和$b$升，是否可以量出$t$升的水？如果可以，请给出具体步骤，其中$a,b,t \in \Zbb_+$

思路：{==装满==}对应$+a$或$+b$，{==倒掉==}对应$-a$或$-b$

是否可以通过若干次$+a$、$+b$、$-a$、$-b$得到$t$？

是否存在整数$x$、$y$使得$ax + by = t$？

前面的例子：$5 \times 3 - 7 \times 2 = 1$

$t$必须是$a,b$最大公约数的倍数，若$t=1$，则$a,b$互素
...
输入：两个输入$a, b \in \Zbb_+$，输出最大公约数 ({==g==}reatest {==c==}ommon {==d==}ivisor, gcd)
...
@import "../codes/gcd.py" {line_begin=0 line_end=5 .left4 .line-numbers}
...
@import "../codes/gcd.py" {line_begin=7 line_end=12 .left4 .line-numbers}
...
@import "../codes/gcd.py" {line_begin=14 line_end=22 .left4 .line-numbers .bottom0}
...
@import "../codes/gcd.py" {line_begin=24 line_end=37 .left4 .line-numbers .top-1 .bottom0}
...
{==循环不变式==} (loop-invariant)：一个满足如下三条性质的的命题
