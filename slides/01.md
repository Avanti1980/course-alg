...
实数{==三歧性==}：$a > b$、$a = b$、$a < b$至少成立一个 <span class="red">✓</span>

函数二歧性：$f(n) = O(g(n))$、$f(n) = \Omega(g(n))$至少成立一个 <span class="red">✗</span>
...
例：$\max \{ f(n), g(n) \} = \Theta(f(n) + g(n))$是否成立

成立，$(f(n) + g(n))/ 2 \le \max \{ f(n), g(n) \} \le f(n) + g(n)$
...
两边可进一步放缩成$\lg n \le \lg (f(n)) \le 2 \lg n$，命题得证
...
证明：根据$\Theta$的对称性，$n = \Theta(k \lg k)$

$\exists c_1, c_2, n_0 > 0$使得对$\forall n \ge n_0$有$0 \le c_1 k \lg k \le n \le c_2 k \lg k$

从而$\lg c_1 + \lg k + \lg (\lg k) \le \lg n \le \lg c_2 + \lg k + \lg (\lg k)$

两边可进一步放缩成$\lg k \le \lg n \le 3 \lg k$

两式相除有$c_1 k / 3 \le n / \lg n \le c_2 k \Longrightarrow n / \lg n = \Theta(k)$

再次利用$\Theta$的对称性，命题得证
