...
贪心法：将问题转化为连续决策，每次决策做贪心选择，只产生一个小规模的子问题
...
- 优点：易设计贪心选择，易分析时间复杂度
- 缺点：结论不总是正确的，正确性证明较难，只对{==部分==}最优化问题有效
...
前一个命题表明选择频率最低的两个字符可以构造最优编码树

后一个命题表明将频率最低的两个字符$x$、$y$合并成$z$后加入字符集合，由此构造出的最优前缀码树再将$z$对应的叶子结点作为内部结点分解成$x$、$y$两个叶结点，依然还是最优编码树
