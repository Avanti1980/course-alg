...
- 每次合并需要查找频率最低的两个结点，线性查找$\Theta(n)$
- 插入合并的结点$\Theta(1)$

<p class="fragment top4" data-fragment-index="1">高级实现：利用优先队列保存结点，总时间复杂度$\Theta(n \lg n)$</p>

<ul class="fragment" data-fragment-index="1">
  <li>初始化优先队列$\Theta(n \lg n)$</li>
...
@import "../codes/huffman.py" {line_begin=3 .left4 .line-numbers .top1 .bottom0-10}
...
<p class="fragment">定理：若所有边权重各不相同，则 MST 唯一</p>
...
@import "../dot/span-tree-proof.dot" {.lefta .right4 .top-22per}
...
@import "../codes/mst-kruskal.py" {line_begin=0 .left4 .line-numbers .top1 .bottom-10}
