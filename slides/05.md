...
引理：对任意排列$\pi$，若存在$i < j$使得$L[\pi(i)] \ge L[\pi(j)]$，记$a = \pi(i)$、$b = \pi(j)$，交换文件$a$、文件$b$后得到的新排列为$\pi'$，则$\Ebb[c(\pi')] \le \Ebb[c(\pi)]$。

<p class="fragment">证明：交换文件$a$、文件$b$后</p>

<ul class="fragment">
  <li>文件$a$的读取开销增加$L[\pi(i+1)] + \cdots + L[\pi(j-1)] + L[\pi(j)]$</li>
  <li>文件$b$的读取开销减少$L[\pi(i)] + L[\pi(i+1)] + \cdots + L[\pi(j-1)]$</li>
  <li>文件$a$、$b$之间的文件的读取开销减少$L[\pi(i)] - L[\pi(j)]$</li>
  <li>由于$L[\pi(i)] \ge L[\pi(j)]$，因此$\Ebb[c(\pi')] \le \Ebb[c(\pi)]$</li>
</ul>
...
<p class="fragment">算法的正确性：设最优排列$\pi^\star$与$L[1, \ldots, n]$的增序不同，第一处不同的位置$\ge k$，交换$\pi^\star[k]$与$k$得到新的排列$\pi'$，由引理知$\Ebb[c(\pi')] \le \Ebb[c(\pi^\star)]$，故$\pi'$也是最优排列，且与增序第一处不同的位置$\ge k+1$，由归纳法可知存在最优排列与增序第一处不同的位置$\ge n$，因此增序就是最优排列</p>
...
引理：对$\forall \pi$，若存在$i < j$使$L[\pi(i)] / F[\pi(i)] \ge L[\pi(j)] / F[\pi(j)]$，记$a = \pi(i)$、$b = \pi(j)$，交换文件$a$、文件$b$后得到的新排列为$\pi'$，则$\Ebb[c(\pi')] \le \Ebb[c(\pi)]$。

<p class="fragment">证明：交换文件$a$、文件$b$后</p>

<ul class="fragment">
  <li>文件$a$的读取开销增加$F[a] (L[\pi(i+1)] + \cdots + L[\pi(j-1)] + L[\pi(j)])$</li>
  <li>文件$b$的读取开销减少$F[b] (L[\pi(i)] + L[\pi(i+1)] + \cdots + L[\pi(j-1)])$</li>
  <li>文件$a$、$b$之间的文件$k$的读取开销减少$F[\pi(k)] (L[\pi(i)] - L[\pi(j)])$</li>
</ul>
...
由归纳法$\{ g_1, ~ g_2, ~ \ldots, g_{i-1}, ~ g_i, ~ \ldots, ~ g_k, ~ c_{k+1}, ~ \ldots, ~ c_l \} $是最大兼容活动集合，由贪心法的选择知$k = m$
