...
若$a_m$在$\Scal_k$中结束最早，则其必在$\Scal_k$的某个最大兼容活动集合中

证明：令$\Acal_k$是$\Scal_k$的最大兼容活动集合，$a_j$在$\Acal_k$中结束最早

若$a_j = a_m$，则结论得证，故设$a_j \ne a_m$，即$a_m$不在$\Acal_k$中
...
令$A'_k = \Acal_k \setminus \{a_j\} \cup \{a_m\}$，则$\Acal'_k$也是兼容的

由于$|\Acal'_k|=|\Acal_k|$，故$\Acal'_k$也是最大兼容活动集合且包含$a_m$
