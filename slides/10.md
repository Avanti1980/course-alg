...
任意流$f$的值不能超过$\Gcal$的任意切割的容量，特别地，任意流$f$的值不能超过最小切割的容量
...
- 利用 DFS 或者 BFS 计算增广路径，运行时间$O(|\Vcal| + |\Ecal|) \in O(|\Ecal|)$
...
$$
\begin{align*}
    \quad \delta_{f'} (s, v) < \delta_f (s, v) \le \delta_f (s, u) + 1 \le \delta_{f'} (s, u) + 1 = \delta_{f'} (s, v) - 1
\end{align*}
$$

$(u,v) \not \in \Ecal_f \Rightarrow f(u,v) = c(u,v)$，$(u,v) \in \Ecal_{f'} \Rightarrow f'(u,v) < c(u,v)$

残存网络$\Gcal_f$中得到的增广路径上边$(v,u)$有流量，抵消了$(u,v)$上的流量，故$(v,u)$是$\Gcal_f$中$s$到$u$的最短路径上的最后一条边
...
对每个匹配的左结点$u \in \Lcal$有$f(s,u) = 1$，而对$\forall (u,v) \in \Ecal \setminus \Mcal$有$f(u,v) = 0$，因此横跨切割$(\Lcal \cup \{s\}, \Rcal \cup \{ t \})$的净流量等于$|\Mcal|$，根据切割的净流量等于流网络的流量，所以$|f|=|\Mcal|$
...
定理：二分图$\Gcal$的一个最大匹配$M$的边数等于其对应的流网络$\Gcal'$中某一最大流$f$的值
