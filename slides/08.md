...
有些问题要求在相对于输入规模呈{==指数增长==}的域中寻找元素

<div class="top-1"></div>

穷举法，检查所有的元素？时间复杂度：指数级
...
{==解的表示==}：$n$元组$(x_1, \ldots, x_n)$，其中$x_i \in \{1, \ldots, n\}$为列号
...
- 左图：{==定长==}$4$元组$(x_1, x_2, x_3, x_4)$，$x_i \in \{0,1\}$表示是否选$w_i$，16 个叶结点对应 16 个可能的解
- 右图：{==变长递增==}元组，例如$(1, 4)$表示选择$w_1$和$w_4$，对应结点 9，整棵树共 16 个结点对应 16 个可能的解

@import "../dot/subset-sum-sol-space.dot" {.center .top2}
...
@import "../dot/subset-sum.dot" {.center .top0}
...
@import "../codes/subset-sum.py" {line_begin=0 line_end=48 .left4 .line-numbers .top0}
